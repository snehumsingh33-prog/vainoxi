...
function render() {
  ...
  adminList.innerHTML = filtered.length ? filtered.map((item) => `<article class="admin-card"><div><span class="status ${item.status}">${item.status}</span><h3>${escapeHtml(item.item_name)}</h3><p>${escapeHtml(item.description)}</p><p><strong>${escapeHtml(item.city)}</strong> · ${escapeHtml(item.whatsapp)}</p></div><div class="admin-actions">${item.status !== 'approved' ? `<button data-action="approve" data-id="${item.id}">Approve</button>` : ''}${item.status !== 'rejected' ? `<button class="reject" data-action="reject" data-id="${item.id}">Reject</button>` : ''}<button class="delete" data-action="delete" data-id="${item.id}">Delete</button></div></article>`).join('') : '<p class="muted">No listings in this view.</p>';
}
